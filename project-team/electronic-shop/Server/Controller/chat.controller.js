...
const createConversation = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(customer_id)) return res.status(400).json({ success: false, message: "Invalid customer_id" });
    if (staff_id && !isValidObjectId(staff_id)) return res.status(400).json({ success: false, message: "Invalid staff_id" });

    const data = await ChatConversation.create({ customer_id, staff_id: staff_id || null, status: status || "open" });
...
const getConversationById = async (req, res) => {
  try {
    ...
    const conversation = await ChatConversation.findById(id).populate("customer_id", "name email img_url").populate("staff_id", "name email img_url").select("-__v").lean();
    if (!conversation) return res.status(404).json({ success: false, message: "Conversation not found" });
    const messages = await ChatMessage.find({ conversation_id: id }).populate("sender_id", "name email img_url").select("-__v").sort({ createdAt: 1 });
...
const createChatMessage = async (req, res) => {
  try {
    ...
    const { sender_id, messages, message, attachments } = req.body;
    if (!conversation_id || !sender_id || (!messages && !message)) {
      return res.status(400).json({ success: false, message: "conversation_id, sender_id and messages are required" });
    }
    if (!isValidObjectId(conversation_id) || !isValidObjectId(sender_id)) return res.status(400).json({ success: false, message: "Invalid conversation_id or sender_id" });
    const conversation = await ChatConversation.findById(conversation_id);
    if (!conversation) return res.status(404).json({ success: false, message: "Conversation not found" });
    const data = await ChatMessage.create({ conversation_id, sender_id, messages: messages || message, attachments: Array.isArray(attachments) ? attachments : [] });
    await ChatConversation.findByIdAndUpdate(conversation_id, { updatedAt: new Date() });
