...
const createTicket = async (req, res) => {
  try {
    ...
    const data = await SupportTicket.create({
      ...
      status: status || "open",
      closedAt: ["closed", "resolved"].includes(status) ? new Date() : null,
    });
...
const getTicketById = async (req, res) => {
  try {
    ...
    const messages = await TicketMessage.find({ ticket_id: id }).populate("sender_id", "name email img_url").select("-__v").sort({ createdAt: 1 });
...
const updateTicketById = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(id)) return res.status(400).json({ success: false, message: "Invalid ticket id" });
    const updateData = {};
    for (const field of ["assigned_staff_id", "subject", "description", "status"]) if (req.body[field] !== undefined) updateData[field] = req.body[field];
    if (["closed", "resolved"].includes(req.body.status)) updateData.closedAt = new Date();
