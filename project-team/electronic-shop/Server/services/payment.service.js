...
// Tao link VietQR tu dong dua tren thong tin ngan hang cau hinh trong .env
const buildVietQrUrl = ({ amount, transferContent }) => {
  ...
  const accountName = process.env.BANK_TRANSFER_ACCOUNT_NAME || "ONLINE TECH SHOP";
...
// Lay thong tin thanh toan theo don hang
const getPaymentByOrder = async (orderId) => {
  ...
  const payment = await PaymentTransaction.findOne({ order_id: orderId })
    .sort({ updated_at: -1 })
    .lean();
...
// Khoi tao giao dich chuyen khoan VietQR
const createBankTransferPayment = async (orderId) => {
  ...
  const payment = await PaymentTransaction.findOneAndUpdate(
    ...
    },
    {
      ...
      account_name: process.env.BANK_TRANSFER_ACCOUNT_NAME || "ONLINE TECH SHOP",
...
// Khoi tao giao dich thanh toan qua cong ZaloPay
const createZaloPayPayment = async (orderId, { clientBaseUrl }) => {
  const order = await Order.findById(orderId);
  if (!order) {
    throw new Error("Order not found");
  }
  ...
  if (!appId || !key1) {
    throw new Error("Missing ZALOPAY_APP_ID or ZALOPAY_KEY1 in Server/.env");
  }

  const amount = toSafeAmount(order.total_amount);
  if (amount <= 0) {
    throw new Error("Order amount must be greater than 0 for ZaloPay payment");
  }

  const appTransId = `${getVietnamDatePrefix()}_${String(order._id).slice(-12)}`;
  const appTime = Date.now();
  const redirectUrl = `${clientBaseUrl}/payment-result/${order._id}?provider=zalopay`;
  ...
  // Callback URL de ZaloPay goi webhook ve Server
  ...
  const isSuccess = zaloResult.return_code === 1 || zaloResult.returncode === 1;
...
// Xu ly ZaloPay Webhook/Callback xac nhan thanh toan tu dong
const handleZaloPayCallback = async ({ data, mac }) => {
  const key2 = process.env.ZALOPAY_KEY2;
  if (!key2) {
    throw new Error("Missing ZALOPAY_KEY2");
  }

  // Kiem tra chu ky MAC de dam bao request thuc su den tu ZaloPay
  ...
  // Cap nhat trang thai thanh toan trong PaymentTransaction
  payment.status = "paid";
  payment.paid_at = new Date();
  payment.provider_order_id = zpTransId ? String(zpTransId) : payment.provider_order_id;
  payment.raw_response = callbackData;
  await payment.save();

  // Cap nhat trang thai thanh toan trong don hang Order
...
// Xac nhan thanh toan chuyen khoan MB/VietQR thu con tu Admin/Staff
const confirmBankTransferPayment = async (orderId) => {
  const order = await Order.findById(orderId);
  if (!order) {
    throw new Error("Order not found");
  }
  ...
  if (!payment) {
    throw new Error("Bank transfer payment transaction not found");
  }
