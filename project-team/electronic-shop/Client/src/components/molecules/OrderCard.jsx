    ...
    <Card className='card-surface mb-3'>
      <Card.Body className='p-4'>
        <div className='d-flex flex-column flex-lg-row justify-content-between gap-3 mb-3'>
          <div>
            <p className='mb-1 text-sm font-bold text-slate-400'>ĐƠN HÀNG {formatOrderCode(order)}</p>
        ...
        <div className='!rounded-4 bg-slate-50 p-3 mb-3'>
          {items.length === 0 ? <p className='mb-0 text-sm text-slate-500'>Không có thông tin sản phẩm.</p> : items.map((item) => (
            <div key={getId(item)} className='d-flex justify-content-between gap-3 border-bottom py-2 last:border-0'>
              <span className='text-sm font-semibold text-slate-700'>{item.product_id?.name || 'Sản phẩm'} x {item.quantity}</span>
        ...
        <div className='d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3'>
          ...
          <div className='d-flex align-items-center gap-3'>
            ...
            {canCancel && <button type='button' className='!rounded-pill border-0 bg-red-50 px-4 py-2 text-sm font-bold text-red-600' onClick={() => onCancel?.(getId(order))}>Hủy đơn</button>}
          </div>
        </div>
      </Card.Body>
    </Card>
