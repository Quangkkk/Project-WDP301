  ...
  const handleSend = async (event) => {
    ...
    try {
      ...
      socketRef.current.emit(
        ...
        (response) => {
          if (!response?.success) {
            setError(response?.message || 'Không gửi được tin nhắn.')
            return
          }
      ...
      setError(getErrorMessage(error, 'Không tải được file chat.'))
    } finally {
          ...
          >
            ...
            {activeTab === 'chat' && (
              <>
                ...
                <form
                  onSubmit={handleSend}
                  className='border-top border-slate-100 bg-white p-2'
                >
                  {selectedFiles.length > 0 && (
                    <div className='mb-2 d-flex flex-wrap gap-1'>
                      {selectedFiles.map((file, index) => (
                        <div
                          key={`${file.name}-${index}`}
                          className='d-flex align-items-center gap-1 !rounded-pill border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] text-slate-700'
                        >
                          <i
                            className={
                  ...
                  <div className='flex items-center gap-2'>
                    <button
                      ...
                      className='d-flex h-9 w-9 align-items-center justify-content-center !rounded-full border border-slate-200 bg-white text-slate-600 transition hover:border-orange-300 hover:bg-orange-50 hover:text-orange-600'
                    ...
                    <input
                      ...
                      className='flex-1 border border-slate-200 !rounded-pill px-3 py-2 text-sm shadow-none focus:border-orange-500 focus:outline-none'
                      disabled={isLoading || isUploading}
                    />

                    <button
                      type='submit'
                      className='d-flex h-9 w-9 align-items-center justify-content-center !rounded-full border-0 bg-orange-500 text-white transition hover:bg-orange-600'
