...
function OrderHistoryPage() {
  ...
  const loadOrders = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await getOrders({ user_id: getUserId(user) })
    ...
    <MainLayout>
      ...
      <section className='page-section'>
        <Container>
          ...
          {isLoading ? <LoadingText /> : !user ? <EmptyState icon='🔐' title='Bạn cần đăng nhập' description='Backend hiện tại yêu cầu user_id để xem đơn hàng.' actionLabel='Login' onAction={() => window.location.assign('/login')} /> : orders.length === 0 ? <EmptyState icon='📦' title='Chưa có đơn hàng' description='Đơn hàng sau khi checkout sẽ xuất hiện ở đây.' /> : orders.map((order) => <OrderCard key={order._id || order.id} order={order} onCancel={handleCancel} />)}
        </Container>
      </section>
    </MainLayout>
  )
