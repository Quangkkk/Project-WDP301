...
function SupportManagementPage() {
  ...
  return <DashboardLayout title='Quản lý yêu cầu hỗ trợ' description='Nhân viên xử lý và đóng yêu cầu hỗ trợ.'><Alert type='danger'>{error}</Alert><Alert type='success'>{message}</Alert><Card className='card-surface'><Card.Body className='p-0'><Table responsive hover className='mb-0'><thead><tr><th className='p-3'>Yêu cầu</th><th>Khách hàng</th><th>Trạng thái</th><th>Cập nhật</th></tr></thead><tbody>{tickets.map((ticket) => { const id = getId(ticket); return <tr key={id}><td className='p-3'><b>{ticket.subject}</b><br /><span className='text-sm text-slate-500'>{ticket.description}</span><br /><span className='text-xs text-slate-400'>{formatDate(ticket.created_at)}</span></td><td>{ticket.user_id?.name || '--'}</td><td><StatusBadge value={ticket.status} /></td><td><div className='d-flex align-items-center gap-2'><SelectField value={draft[id]} options={[{ value: 'pending', label: 'Chờ xử lý' }, { value: 'open', label: 'Đang mở' }, { value: 'closed', label: 'Đã đóng' }]} onChange={(e) => setDraft((p) => ({ ...p, [id]: e.target.value }))} /><Button size='sm' onClick={() => handleSave(id)}>Lưu</Button></div></td></tr> })}</tbody></Table></Card.Body></Card></DashboardLayout>
}
